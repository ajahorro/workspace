...
BEGIN
    ...
    ('00000000-0000-0000-0000-000000000000', uid_staff1, 'authenticated', 'authenticated', 'staff1@speed.way', crypt('staff123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now()),
    ('00000000-0000-0000-0000-000000000000', uid_staff2, 'authenticated', 'authenticated', 'staff2@speed.way', crypt('staff123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now()),
    ...
    ('00000000-0000-0000-0000-000000000000', uid_admin, 'authenticated', 'authenticated', 'admin@speed.way', crypt('admin123', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now());
    ...
    (uid_staff1, 'Staff One', 'staff1@speed.way', 'STAFF', now()),
    (uid_staff2, 'Staff Two', 'staff2@speed.way', 'STAFF', now()),
    (uid_cust1, 'Customer One', 'customer1@renew.com', 'CUSTOMER', now()),
    (uid_cust2, 'Customer Two', 'customer2@renew.com', 'CUSTOMER', now()),
    (uid_admin, 'Super Admin', 'admin@speed.way', 'ADMIN', now());
END $$;
